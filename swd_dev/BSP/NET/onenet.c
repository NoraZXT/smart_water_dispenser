...
//产品ID
#define PROID			"mHB0WKVSN0"
//设备密钥
#define ACCESS_KEY		"d1RicWhsQzZLTEdQc0VHc05UWG5LSTVWOUNSUEplRmM="
//设备名称
...
extern unsigned char SendWaitForAsk;
...
extern unsigned char LACK_WATER, MODE, PUMP_status;
unsigned char OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"set_temp_h\":{\"value\":%d},", SET_TEMP_HIGH);
	strcat(buf, text);
    
	memset(text, 0, sizeof(text));
	sprintf(text, "\"set_temp_l\":{\"value\":%d},", SET_TEMP_LOW);
	strcat(buf, text);    
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"water_level\":{\"value\":%s}", LACK_WATER ? "true" : "false");
	strcat(buf, text);
...
void OneNet_RevPro(unsigned char *cmd)
{
    ...
    cJSON *raw_json, *params_json, *mode_json, *pump_json, *th_json, *tl_json;
	...
	switch(type)
	{
		case MQTT_PKT_PUBLISH:																//接收的Publish消息
			...
			if(result == 0)
			{
                ...
                tl_json = cJSON_GetObjectItem(params_json,"set_temp_l");
                th_json = cJSON_GetObjectItem(params_json,"set_temp_h");
                if(mode_json != NULL)
                {
                    //不缺水情况下才能对加热模块进行操作
                    if(!LACK_WATER)
                    {
                        if(mode_json->type == cJSON_True) MODE = 1;
                        else MODE = 0;                            
                    }
                }
                if(pump_json != NULL)
                {   
                    if(pump_json->type == cJSON_True)
                    {
                        //在不缺水情况下才能对水泵进行操作
                        if(!LACK_WATER)
                        {   
                            //常温模式或加热模式温度大于下限时才可以开启水泵
                            if((MODE == 0) || (MODE == 1 && Temp >= SET_TEMP_LOW))  
                            {
                                PUMP(1);
                                PUMP_status = 1;
                            }
                ...
                if(tl_json != NULL)
                {
                    if(tl_json->valueint < 0)
                        SET_TEMP_LOW = 0;
                    else if((tl_json->valueint + 5) >= SET_TEMP_HIGH)    //固定下限至少比上限小5，防止出现下限比上限搞的情况
                        SET_TEMP_LOW = SET_TEMP_HIGH - 5;
                    else
                        SET_TEMP_LOW = tl_json->valueint;
                }
                if(th_json != NULL)
                {
                    if(th_json->valueint > 80)
                        SET_TEMP_HIGH = 80;
                    else if((SET_TEMP_LOW + 5) >= th_json->valueint)   //固定下限至少比上限小5，防止出现下限比上限搞的情况
                        SET_TEMP_HIGH = SET_TEMP_LOW + 5;
                    else                   
                        SET_TEMP_HIGH = th_json->valueint;
                }
                
                cJSON_Delete(raw_json);   //删包防止溢出             
